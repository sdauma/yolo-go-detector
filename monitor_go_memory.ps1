Write-Host "=== Go 内存监控脚本 ===" -ForegroundColor Green

$maxRSS = 0
$rssValues = @()
$startTime = Get-Date

Write-Host "开始监控 Go 进程内存使用..." -ForegroundColor Cyan
Write-Host "按 Ctrl+C 停止监控" -ForegroundColor Yellow

try {
    while ($true) {
        $process = Get-Process -Name "benchmark_go_std_intra4*" -ErrorAction SilentlyContinue
        ...
        if ($process) {
            ...
            if ($currentWS -gt $maxRSS) {
                ...
                Write-Host "[峰值] RSS: $maxRSS_MB MB" -ForegroundColor Red
            }
            ...
            Write-Host "[实时] 运行时间: $($elapsed.ToString('mm\:ss')), RSS: $currentWS_MB MB" -ForegroundColor Cyan
        } else {
            Write-Host "[状态] 未找到 Go 进程" -ForegroundColor Gray
        }
