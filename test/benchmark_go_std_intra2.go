...
func main() {
	fmt.Println("===== Go 标准性能测试 (intra=2, inter=1, yolo11x) =====")
	...
	// 保存结果
	file, err := os.Create("go_benchmark_std_intra2.txt")
	if err == nil {
		file.WriteString("===== Go 标准性能测试结果 (intra=2, inter=1, yolo11x) =====\n")
		file.WriteString("测试配置: warmup=10, runs=30, batch=1, concurrency=1\n")
		fmt.Fprintf(file, "Intra-op threads: 2\n")
		...
		file.Close()
		fmt.Println("\nResults saved to go_benchmark_std_intra2.txt")
	}
