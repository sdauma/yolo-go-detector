...
for %%F in (paper_full_benchmark go_session_pool_ablation go_baseline_minimal go_reinforced_benchmark go_reinforced_yolo11n go_reinforced_benchmark_small go_pure_inference_benchmark go_concurrent_stress_fixed cold_start_benchmark go_memory_breakdown go_memory_copy_overhead thread_config_benchmark thread_config_benchmark_yolo11n go_session_creation_benchmark go_output_consistency go_cpu_monitoring go_warmup_effect go_advanced_session_supplementary go_performance_diagnostic go_memory_standardization go_cold_start_decomposition go_batch_inference go_architecture_benchmark go_concurrent_architecture_comparison go_72h_stability go_arena_ablation go_arena_ablation_fixed_concurrency go_supplementary_ablation go_session_lifecycle_repro go_session_pool_fault_injection) do (
    set /a TOTAL+=1
    echo [BUILD] %%F.exe ...
...
if !ERRORLEVEL!==0 (set /a BUILT+=1 && echo [OK]   batch_verify_pure.exe) else (set /a FAILED+=1 && echo [FAIL] batch_verify_pure.exe)
popd
...
"%MSVC_BIN%\cl.exe" /EHsc /O2 /utf-8 /std:c++17 /I"%MSVC_INC%" /I"%SDK_INC%\ucrt" /I"%SDK_INC%\um" /I"%SDK_INC%\shared" /I"%ORT_INC%" cpp_baseline_benchmark.cpp /link /LIBPATH:"%MSVC_LIB%" /LIBPATH:"%SDK_LIB%\ucrt\x64" /LIBPATH:"%SDK_LIB%\um\x64" /LIBPATH:"%ORT_LIB%" onnxruntime.lib /OUT:cpp_baseline_benchmark.exe
...
REM --- Build C++ Arena Ablation (MSVC) ---
echo.
echo ========================================
echo   Build C++ Arena Ablation (MSVC)
echo ========================================
setlocal disabledelayedexpansion
...
if not exist "%MSVC_BIN%\cl.exe" goto :arena_no_msvc
echo [BUILD] cpp_arena_ablation_benchmark.exe ...
...
if not %ERRORLEVEL%==0 goto :arena_build_fail
echo [OK]   cpp_arena_ablation_benchmark.exe
goto :arena_build_done
:arena_no_msvc
echo [WARN] MSVC (cl.exe) not found, skipping C++ Arena Ablation build
if exist cpp_arena_ablation_benchmark.exe (
    echo [INFO] cpp_arena_ablation_benchmark.exe already exists, skipping build
) else (
    echo [WARN] cpp_arena_ablation_benchmark.exe not found
)
goto :arena_build_done
:arena_build_fail
echo [FAIL] cpp_arena_ablation_benchmark.exe
:arena_build_done
endlocal