// thread_config_benchmark_yolo11n.go
// Go 线程配置性能测试
//
// 重要声明（P0原则）：
...
func main() {
	...
	for _, numThreads := range threadConfigs {
		...
		for testIdx := 1; testIdx <= testCount; testIdx++ {
			...
			// 创建输出张量（YOLO11 系列输出形状通常为 [1, 84, 8400]）
		...
		// 保存详细日志
		logPath := filepath.Join(basePath, "results", fmt.Sprintf("go_thread_yolo11n_%d_detailed_log.txt", numThreads))
		...
		// 保存单个配置的结果
		resultPath := filepath.Join(basePath, "results", fmt.Sprintf("go_thread_yolo11n_%d_result.txt", numThreads))
		fmt.Printf("\n保存结果到: %s\n", resultPath)
	...
	// 保存所有线程配置的综合结果
	comprehensiveResultPath := filepath.Join(basePath, "results", "go_thread_config_yolo11n_comprehensive.txt")
	...
	if err != nil {
		...
		fmt.Fprintf(file, "%-20s %-15s %-12s %-12s %-10s %-15s %-15s %-15s %-15s %-15s\n",
			"线程配置", "平均延迟(ms)", "标准差(ms)", "变异系数(%)", "FPS", "P50延迟(ms)", "P90延迟(ms)", "P99延迟(ms)", "Start PM(MB)", "Stable PM(MB)")
