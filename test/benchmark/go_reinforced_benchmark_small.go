// go_reinforced_benchmark_small.go
// Go 强化测试（YOLO11n 轻模型）- 10轮×200次推理
//
// 技术说明：
...
func main() {
	...
	// 保存详细日志
	logPath := filepath.Join(basePath, "results", "go_reinforced_small_detailed_log.txt")
	...
	// 保存平均值结果
	resultPath := filepath.Join(basePath, "results", "go_reinforced_small_result.txt")
	...
	fmt.Fprintf(resultFile, "===== Go 轻模型强化测试结果（10轮运行） =====\n")
	for i, r := range results {
	...
	// 保存原始延迟数据（用于分析）
	latencyDataPath := filepath.Join(basePath, "results", "go_reinforced_small_latency_data.txt")
