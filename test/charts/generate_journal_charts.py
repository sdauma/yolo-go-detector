...
def generate_latency_boxplot():
    ...
    # 从强化测试结果文件读取数据
    go_latencies = read_latency_data('../../results/go_reinforced_result.txt')
    python_latencies = read_latency_data('../../results/python_reinforced_result.txt')
                ...
                for separator in [':', '：']:
                    if separator in line:
                        try:
                            ...
                            pass
            elif '峰值RSS' in line and current_arch == 'session_pool' and current_concurrency is not None:
                # 支持中文冒号和英文冒号
                for separator in [':', '：']:
                    if separator in line:
                        try:
                ...
                if '架构=SessionPool' in line:
                    ...
                    for part in parts:
                        if '池大小=' in part:
                            ...
                            try:
                                # 提取数字部分（去除单位MB）
                                value_str = part.split('=')[1].strip().split(' ')[0]
...
def generate_memory_scalability():
    ...
    # 从文件读取数据
    go_memory = read_memory_data('../../results/go_architecture_comparison.txt', concurrency)
    python_memory = read_python_memory_data('../../results/python_architecture_comparison.txt', concurrency)
    ...
    # 添加标注（不使用颜色描述）
    ax.annotate('Go: 增长平缓\n稳定在 60MB 左右', 
                xy=(8, 60.77), xytext=(6, 100),
                fontsize=10, bbox=dict(boxstyle='round', facecolor='white', alpha=0.9),
                arrowprops=dict(arrowstyle='->', color='black'))
    
    ax.annotate('Python: 线性增长\n12并发时达到 5965.93MB', 
                xy=(12, 5965.93), xytext=(8, 4500),
                fontsize=10, bbox=dict(boxstyle='round', facecolor='white', alpha=0.9),
                arrowprops=dict(arrowstyle='->', color='black'))
...
def generate_cold_start_decomposition():
    ...
    # 从冷启动测试结果文件读取数据
    go_data = read_cold_start_data('../../results/go_cold_start_result.txt')
    python_data = read_cold_start_data('../../results/python_cold_start_result.txt')
...
# 从冷启动测试结果文件读取数据
def read_cold_start_data(file_path):
    """从冷启动测试结果文件读取分解数据"""
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            lines = f.readlines()
        ...
        if cold_start_time == 0 or stable_time == 0:
            raise FileNotFoundError(f"未从文件 {file_path} 中读取到冷启动时间或稳定状态时间，请检查文件格式是否正确")
...
def generate_perf_memory_scatter():
    ...
    # 从基准测试结果文件读取数据
    go_baseline = read_baseline_data('../../results/go_baseline_result.txt')
    python_baseline = read_baseline_data('../../results/python_baseline_result.txt')
...
def read_baseline_data(file_path):
    ...
    try:
        ...
        for line in lines:
            line = line.strip()
            if '平均延迟' in line and '标准差' not in line:
                # 支持中文冒号和英文冒号
                for separator in [':', '：']:
                    if separator in line:
                        try:
...
def generate_reinforced_performance():
    ...
    # 从强化测试结果文件读取数据
    go_data = read_reinforced_performance_data('../../results/go_reinforced_result.txt')
    python_data = read_reinforced_performance_data('../../results/python_reinforced_result.txt')
...
def generate_reinforced_memory():
    ...
    # 从强化测试结果文件读取数据
    go_data = read_reinforced_memory_data('../../results/go_reinforced_result.txt')
    python_data = read_reinforced_memory_data('../../results/python_reinforced_result.txt')
...
def generate_reinforced_cold_start():
    ...
    # 从冷启动测试结果文件读取数据
    go_cold_start = read_cold_start_time('../../results/go_cold_start_result.txt')
    python_cold_start = read_cold_start_time('../../results/python_cold_start_result.txt')
    
    go_reinforced = read_reinforced_performance_data('../../results/go_reinforced_result.txt')
    python_reinforced = read_reinforced_performance_data('../../results/python_reinforced_result.txt')
...
def generate_output_consistency():
    ...
    try:
        # 尝试读取输出一致性测试结果文件
        go_output, python_output = read_output_consistency_data('../../results/go_output_consistency_result.txt',
                                                               '../../results/python_output_consistency_result.txt')
        
        # 检查数据读取是否成功
        if go_output is None or python_output is None:
            print("警告：无法读取输出一致性测试结果文件，跳过生成输出一致性对比图")
            print("  - ../../results/go_output_consistency_result.txt")
            print("  - ../../results/python_output_consistency_result.txt")
            return
