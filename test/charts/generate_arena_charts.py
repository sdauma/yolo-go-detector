...
def parse_arena_result(filepath):
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            elif current and '峰值RSS:' in line:
                current['peak_rss'] = float(line.split(':')[1].strip().split(' ')[0])
            elif current and 'RSS漂移:' in line:
...
for i, metric in enumerate(['throughput', 'peak_rss', 'rss_drift']):
    ...
    bars1 = ax.bar([xi - width/2 for xi in x], go_vals_on, width, label='Arena ON', color='#4472C4', edgecolor='black', linewidth=0.5)
    bars2 = ax.bar([xi + width/2 for xi in x], go_vals_off, width, label='Arena OFF', color='#ED7D31', edgecolor='black', linewidth=0.5, hatch='//')
    ...
    if metric == 'throughput':
        ...
        ax.set_ylabel('峰值 RSS (MB)', fontsize=10)
        ax.set_title('(b) 峰值内存对比', fontsize=11)
    elif metric == 'rss_drift':
        ax.set_ylabel('RSS 漂移 (MB)', fontsize=10)
...
x2 = range(len(groups))
bars3 = ax2.bar([xi - width/2 for xi in x2], on_vals, width, label='Arena ON', color='#4472C4', edgecolor='black', linewidth=0.5)
bars4 = ax2.bar([xi + width/2 for xi in x2], off_vals, width, label='Arena OFF', color='#ED7D31', edgecolor='black', linewidth=0.5, hatch='//')

ax2.set_xticks(x2)
ax2.set_xticklabels(groups, fontsize=11)
ax2.set_ylabel('RSS 漂移 (MB)', fontsize=11)
ax2.set_title('图 4-补充(b)  跨语言负对照验证：Arena 开关对 RSS 漂移的影响', fontsize=12)
...
ax2.text(0.05, (go_us_on + go_us_off)/2, f'下降{(1-go_us_off/go_us_on)*100:.1f}%', color='red', fontsize=9, va='center')
...
print('\nGo 侧:')
for d in go_data:
    print(f"  {d['arch']} Arena={d['arena']}: 吞吐={d['throughput']:.3f}, 峰值RSS={d['peak_rss']:.1f}MB, 漂移={d['rss_drift']:.2f}MB")
